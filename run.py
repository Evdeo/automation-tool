...
entry to drive two apps in one test:
    ...
    # then click(window.notepad, ...) and click(window.calc, ...)
...
sends keys to whichever control currently has focus):
    ...
    window.open(name) / close(name) / get(name)   manage app lifecycle
...
Each state function takes `data` (scratch carried between states) and
returns `(next_state, data)`. Returning `(None, data)` ends the run.
Live windows live on `core.window` — not on `data`.
...
def state_new_tab(data):
    ...
    """
    click_when_enabled(window.notepad, FILE_MENU)
    if not wait_visible(window.notepad, NEW_TAB, timeout=5):
        log("results", "new_tab_failed", "NEW_TAB never appeared")
        return "close", data
    click(window.notepad, NEW_TAB)
    # The menu should close after the click — confirm before next state.
    wait_gone(window.notepad, NEW_TAB, timeout=3)
    return "zoom_in", data
...
def state_zoom_in(data):
    ...
    click_when_enabled(window.notepad, VIEW_MENU)
    wait_visible(window.notepad, ZOOM, timeout=5)
    click_when_enabled(window.notepad, ZOOM)
    wait_visible(window.notepad, ZOOM_IN, timeout=5)
    click(window.notepad, ZOOM_IN)
...
def state_zoom_out(data):
    ...
    wait_visible(window.notepad, ZOOM, timeout=5)
    click_when_enabled(window.notepad, ZOOM)
    wait_visible(window.notepad, ZOOM_OUT, timeout=5)
    click(window.notepad, ZOOM_OUT)
...
def state_type_time(data):
    ...
    fill(window.notepad, EDITOR, now())
    return "extra", data
...
def state_extra(data):
    ...
    click_after(window.notepad, EDITOR, delay=0.1)
    return "verify", data
...
def state_verify(data):
    ...
    """
    wait_enabled(window.notepad, FILE_MENU, timeout=5)

    targets = [FILE_MENU, VIEW_MENU, EDITOR]
    visible = each(is_visible, window.notepad, targets)
    enabled = each(is_enabled, window.notepad, targets)
    info = read_info(window.notepad, EDITOR)
    color = check_color(window.notepad, FILE_MENU)
    # Self-match: tolerance=30 so theme drift (light/dark mode) doesn't
    # fail the check. The bool is logged for the user to inspect.
    color_match = is_color(window.notepad, FILE_MENU, color, tolerance=30)
...
def state_save(data):
    ...
    `key("enter")` — not `hotkey(window.notepad, "enter")` — sends the
    ...
    with no_dismiss():
        hotkey(window.notepad, "ctrl", "shift", "s")
        wait(0.8)
        type(target)
        wait(0.2)
        key("enter")
...
def state_close(data):
    ...
    wait_visible(window.notepad, CLOSE_TAB, timeout=3)
